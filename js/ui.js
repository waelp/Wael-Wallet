    ...
    <div class="header">
        <h1>${Lang.t('settings')}</h1>
            ...
            <div class="card">
                ...
                <div style="padding: 12px 0; border-bottom: 0.5px solid var(--ios-gray5);">
                    <div style="margin-bottom: 10px;">${Lang.t('bg_color')}</div>
                    <div style="display: flex; gap: 10px; overflow-x: auto; padding-bottom: 5px;">
                        <div onclick="app.setBackground('bg-default')" style="width: 30px; height: 30px; border-radius: 50%; background: #F2F2F7; border: 1px solid #ccc; cursor: pointer;"></div>
                        <div onclick="app.setBackground('bg-blue')" style="width: 30px; height: 30px; border-radius: 50%; background: #eef2ff; border: 1px solid #ccc; cursor: pointer;"></div>
                        <div onclick="app.setBackground('bg-green')" style="width: 30px; height: 30px; border-radius: 50%; background: #f0fdf4; border: 1px solid #ccc; cursor: pointer;"></div>
                        <div onclick="app.setBackground('bg-rose')" style="width: 30px; height: 30px; border-radius: 50%; background: #fff1f2; border: 1px solid #ccc; cursor: pointer;"></div>
                        <div onclick="app.setBackground('bg-slate')" style="width: 30px; height: 30px; border-radius: 50%; background: #f8fafc; border: 1px solid #ccc; cursor: pointer;"></div>
                        <div onclick="app.setBackground('bg-dark')" style="width: 30px; height: 30px; border-radius: 50%; background: #0f172a; border: 1px solid #ccc; cursor: pointer;"></div>
                    </div>
                </div>
                ...
                <div class="list-item" onclick="app.navigate('reports')">
                    <div class="flex-row" style="width: 100%;">
                        ...
                        <i data-lucide="bar-chart-2" style="color: var(--ios-blue);"></i>
                    </div>
                </div>
...
`;
    ...
    renderReports() {
        const now = new Date();
        const monthly = Store.getMonthlyReport(now.getMonth(), now.getFullYear());
        const quarterly = Store.getQuarterlyReport();
        const expected = Store.getExpectedExpenses();

        const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

        return `
            <div class="header flex-row">
                <h1>${Lang.t('reports')}</h1>
                <button class="btn btn-secondary" style="width: auto; padding: 8px 16px;" onclick="app.navigate('settings')">
            ...
            <!-- Monthly Summary -->
            <div class="card">
                <h3>${Lang.t('monthly_report')} (${monthNames[now.getMonth()]})</h3>
                <div class="grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px;">
                    <div>
                        <div class="text-caption">${Lang.t('income')}</div>
                        <div class="amount text-green">${Utils.formatCurrency(monthly.totalIncome)}</div>
                    </div>
                    <div>
                        <div class="text-caption">${Lang.t('expenses')}</div>
                        <div class="amount text-red">${Utils.formatCurrency(monthly.totalExpenses)}</div>
                    </div>
                    <div style="grid-column: span 2; border-top: 1px solid var(--ios-gray5); padding-top: 8px;">
                        <div class="text-caption">Net</div>
                        <div class="amount ${monthly.net >= 0 ? 'text-green' : 'text-red'}">${Utils.formatCurrency(monthly.net)}</div>
                    </div>
                </div>
            </div>
            ...
            <!-- Quarterly Trend -->
            <div class="card">
                <h3>${Lang.t('quarterly_report')}</h3>
                <div style="margin-top: 12px;">
                    ${quarterly.map(q => `
                        <div class="flex-row" style="margin-bottom: 8px; font-size: 14px;">
                            <span style="width: 40px;">${monthNames[q.month]}</span>
                            <div style="flex: 1; margin: 0 10px; height: 6px; background: var(--ios-gray5); border-radius: 3px; overflow: hidden;">
                                <div style="width: ${Math.min((q.totalExpenses / (q.totalIncome || 1)) * 100, 100)}%; background: var(--ios-red); height: 100%;"></div>
                            </div>
                            <span class="amount">${Utils.formatCurrency(q.totalExpenses)}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
            ...
            <!-- Expected Expenses -->
            <div class="card">
                <h3>${Lang.t('expected_expenses')}</h3>
                <div style="margin-top: 12px;">
                    <div class="flex-row" style="margin-bottom: 12px;">
                        <span class="text-secondary">Total Installments</span>
                        <span class="amount text-orange">${Utils.formatCurrency(expected.totalInstallments)}</span>
                    </div>
                    ${expected.debts.map(d => `
                        <div class="list-item" style="padding: 8px 0; font-size: 14px;">
                            <div>${d.name}</div>
                            <div class="amount">${Utils.formatCurrency(d.monthly_installment)}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
