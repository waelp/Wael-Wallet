const Store = {
    ...
    getEmptyData() {
        return {
            ...
            debtPayments: [],
            savings: [],
            settings: { currency: 'SAR', salaryDay: 1, pin: '0000' }
        };
    },
    ...
    loadData() {
        ...
        if (stored) {
            ...
            // Ensure settings exist if loading old data
            if (!this.data.settings) this.data.settings = {};
            if (!this.data.settings.pin) this.data.settings.pin = '0000';
        } else {
    ...
    getMonthlyReport(month, year) {
        const income = this.data.income.filter(i => {
            const d = new Date(i.date);
            return d.getMonth() === month && d.getFullYear() === year;
        });
        const expenses = this.data.expenses.filter(e => {
    ...
    getQuarterlyReport() {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const data = this.getMonthlyReport(m, y);
            report.push({
                month: m,
                year: y,
                ...data
    ...
    getExpectedExpenses() {
        ...
        const totalInstallments = debts.reduce((sum, d) => sum + parseFloat(d.monthly_installment), 0);

        // Add other recurring expenses logic here if needed
